...
import { readRoutes, writeRoutes } from "@/lib/routes/storage";
import type { SavedRoute, Waypoint } from "@/lib/routes/storage";

const MAX_STOPS = 12;
const WALK_OPTIONS: WalkOption[] = ["no-stairs", "recommend", "main-road", "shortest"];
...
export function RoutePlanner({ candidates, initialStops = [], pickRequest = null }: RoutePlannerProps) {
  ...
  const [notice, setNotice] = useState("지도에서 장소를 누르거나 빈 곳을 눌러 방문할 지점을 담아 보세요.");
  ...
  const handleOffRoute = useCallback((position: LatLng, legIndex: number) => {
    setStops((previous) => {
      ...
      return [
        { id: `current-${Date.now()}`, kind: "custom", lat: position.lat, lng: position.lng, name: "현재 위치" },
        ...remaining,
      ];
    });
  ...
  const { geo, progress } = useNavigation(route, guiding, handleOffRoute, geoToken);
  ...
  const addPlace = useCallback((place: MapPlace) => {
    ...
    setNotice(`${place.name}을(를) ${current.length + 1}번째 지점으로 담았습니다.`);
  }, []);
  ...
  const addMapPoint = useCallback((point: LatLng) => {
  ...
  function optimize() {
    if (stops.length < 4) {
      setNotice("순서를 최적화하려면 4곳 이상 담아 주세요. 3곳 이하는 순서를 바꿔도 총 거리가 같습니다.");
      return;
    }
  ...
  function saveCourse() {
    if (!route || stops.length < 2) {
      setNotice("저장할 경로가 없습니다. 두 곳 이상을 담아 주세요.");
      return;
    }
    const saved = toSavedRoute(stops, route, option);
    writeRoutes([saved, ...readRoutes()]);
    setNotice(`"${saved.title}" 경로를 마이페이지에 저장했습니다.`);
  }

  const sourceNotice = useMemo(() => {
    if (error) return { tone: "danger" as const, text: error };
    if (source === "fallback" || source === "partial") {
      return { tone: "warn" as const, text: message ?? "일부 구간을 직선으로 표시합니다." };
    }
    return null;
  ...
  return (
    <div className="route-planner">
      <div className="route-planner-map">
        ...
        />
        {loading && !rerouting && <p className="route-map-badge">보행 경로를 탐색하는 중…</p>}
      </div>
        ...
        {guiding && route ? (
          ...
          <>
            <div className="planner-head">
              <h3>방문할 곳 고르기</h3>
              <p className="muted">
                지도의 마커를 누르면 그 장소가, 빈 곳을 누르면 그 지점이 목록에 담깁니다. 담은 순서대로 이동합니다.
              </p>
            </div>
            ...
            <div className="planner-toggles">
              <label>
                <input type="checkbox" checked={fixStart} onChange={(event) => setFixStart(event.target.checked)} />
                출발지 고정
              </label>
              <label>
                <input type="checkbox" checked={fixEnd} onChange={(event) => setFixEnd(event.target.checked)} />
                도착지 고정
              </label>
            </div>
              ...
              {stops.map((stop, index) => (
                <li key={stop.id}>
                  ...
                  <div className="planner-stop-body">
                    ...
                    <small>
                      {index === 0 ? "출발" : index === stops.length - 1 ? "도착" : "경유"}
                      {stop.subtitle ? ` · ${stop.subtitle}` : ""}
                    </small>
                  </div>
                  <div className="planner-stop-actions">
                    <button type="button" onClick={() => move(index, -1)} disabled={index === 0} aria-label={`${stop.name} 순서 올리기`}>
                      ↑
                    </button>
                    <button
                      type="button"
                    ...
                    <button type="button" className="is-danger" onClick={() => remove(index)} aria-label={`${stop.name} 삭제`}>
                      ✕
                    </button>
                  </div>
                </li>
              ))}
              {stops.length === 0 && <li className="planner-empty">아직 담은 지점이 없습니다.</li>}
            </ol>

            <div className="planner-actions">
              <button className="small-btn" type="button" onClick={optimize} disabled={stops.length < 3}>
                순서 자동 최적화
              </button>
              <button className="small-btn" type="button" onClick={refresh} disabled={stops.length < 2}>
                경로 다시 탐색
              </button>
              <button className="small-btn" type="button" onClick={clearAll} disabled={stops.length === 0}>
                전체 비우기
              </button>
            </div>
            ...
            {sourceNotice && (
              <p className={`planner-notice is-${sourceNotice.tone}`} role="alert">
                {sourceNotice.text}
              </p>
            )}
            ...
            <div className="planner-cta">
              <button className="primary-action" type="button" onClick={startGuiding} disabled={!route || loading}>
                경로 안내 시작
              </button>
              <button className="secondary-action" type="button" onClick={saveCourse} disabled={!route}>
                마이페이지에 저장
              </button>
            </div>
          </>
...
function toSavedRoute(stops: RoutePoint[], route: WalkRoute, option: WalkOption): SavedRoute {
  const today = new Date().toISOString().slice(0, 10).replaceAll("-", ".");
  const waypoints: Waypoint[] = stops.map((stop, index) => {
    const leg = route.legs[index];
    return {
      id: index + 1,
      lat: stop.lat,
      lng: stop.lng,
      meta: index === 0 ? "지도에서 직접 선택한 출발지" : "지도에서 직접 선택한 지점",
      name: stop.name,
      next: leg ? `다음 구간 ${formatDistance(leg.distance)} · ${formatDuration(leg.duration)}` : "도착",
      role: index === 0 ? "출발" : index === stops.length - 1 ? "도착" : "경유",
      tags: [WALK_OPTION_LABELS[option]],
    };
  });
  ...
  return {
    ...
    editedDate: today,
    grade: route.warnings.length === 0 ? "A (양호)" : "B+ (주의 구간 있음)",
    id: `custom-${Date.now()}`,
    kind: "내가 만든 경로",
    memo: route.warnings.length > 0 ? route.warnings.join(" / ") : "지도에서 직접 만든 경로입니다.",
    parking: stops[0]?.name ?? "미지정",
    parkingMeta: "출발지로 지정한 지점",
    parkingStatus: "실시간 정보 없음",
    profile: WALK_OPTION_LABELS[option],
    savedDate: today,
    stops: stops.length,
    tags: [WALK_OPTION_LABELS[option], `${stops.length}곳 경유`],
    title: `${stops[0]?.name ?? "출발"} → ${stops[stops.length - 1]?.name ?? "도착"}`,
    walkOption: option,
    waypoints,
  };
}
