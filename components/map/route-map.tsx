...
import type { LatLng, RoutePoint, RouteStep, WalkRoute } from "@/lib/routing/types";
...
export function RouteMap({
  ...
  useEffect(() => {
    ...
    loadKakaoMaps(getKakaoMapAppKey())
      .then((maps) => {
        ...
        maps.event.addListener(mapRef.current, "click", (event: KakaoMouseEvent) => {
          if (!interactiveRef.current || !mapClickRef.current || !event?.latLng) return;
          mapClickRef.current({ lat: event.latLng.getLat(), lng: event.latLng.getLng() });
        });
        setStatus("ready");
      })
      .catch((error: unknown) => {
  ...
  useEffect(() => {
    ...
    candidateOverlaysRef.current = candidates.map((place) => {
  ...
  }, [candidates, onCandidateSelect, status]);
  ...
  useEffect(() => {
    ...
    selectedOverlaysRef.current = selected.map((point, index) => {
      ...
      const role = index === 0 ? "start" : index === selected.length - 1 ? "goal" : "via";
      ...
      polylinesRef.current = route.legs
        ...
        .map((leg, index) => {
          ...
          return new maps.Polyline({
            map,
            path: leg.path.map((point) => new maps.LatLng(point.lat, point.lng)),
            strokeColor: leg.fallback ? FALLBACK_COLOR : dimmed ? ROUTE_DIM_COLOR : ROUTE_COLOR,
  ...
  useEffect(() => {
    ...
    const position = new maps.LatLng(highlightStep.position.lat, highlightStep.position.lng);
    const element = document.createElement("div");
  ...
  return (
    <div className={`route-map-shell ${className ?? ""}`.trim()}>
      <div className="route-map-canvas" ref={containerRef} role="application" aria-label={ariaLabel} />
      {status === "loading" && <div className="map-fallback">카카오맵을 불러오는 중입니다.</div>}
      {status === "error" && (
        <div className="map-fallback map-fallback-error">
          <strong>지도를 표시할 수 없습니다</strong>
          <p>{errorMessage}</p>
          <p className="muted">
            NEXT_PUBLIC_KAKAO_MAP_APP_KEY 를 설정하고 카카오 개발자 콘솔에 현재 도메인을 등록해 주세요.
          </p>
        </div>
      )}
    </div>
