...
import type { LatLng, RouteLeg, RoutePoint, RouteStep, StepKind, WalkOption } from "./types";
...
const PEDESTRIAN_URL = "https://apis.openapi.sk.com/tmap/routes/pedestrian?version=1";
 ...
 * 선택한 옵션이 거부되면 기본 추천 옵션으로 한 번 더 시도합니다.
 */
export async function fetchWalkLeg(
  ...
  try {
    return parseFeatureCollection(await request(appKey, from, to, option, speed), from, to);
  } catch (error) {
    if (option === "recommend" || !(error instanceof TmapRouteError)) throw error;
    // 계단 제외 같은 옵션을 지원하지 않는 구간일 수 있으므로 추천 경로로 한 번 더 시도합니다.
    return parseFeatureCollection(await request(appKey, from, to, "recommend", speed), from, to);
  }
}

/** 경로탐색이 불가능할 때 쓰는 직선 근사 구간. */
export function straightLeg(from: RoutePoint, to: RoutePoint, speed: number): RouteLeg {
  const distance = haversine(from, to);
  const duration = Math.round((distance / ((speed * 1000) / 3600)) || 0);
  return {
    distance,
    duration,
    fallback: true,
...
async function request(
  ...
  try {
    ...
    if (!response.ok) {
      throw new TmapRouteError(readErrorMessage(payload) || `TMAP 경로탐색 실패 (HTTP ${response.status})`);
    }
    ...
    throw new TmapRouteError(error instanceof Error ? error.message : "TMAP 경로탐색 중 오류가 발생했습니다.");
  } finally {
    clearTimeout(timer);
  }
}
...
function parseFeatureCollection(payload: unknown, from: RoutePoint, to: RoutePoint): RouteLeg {
...
function readFeatures(payload: unknown): TmapFeature[] {
  ...
  return features.filter((feature): feature is TmapFeature => Boolean(feature) && typeof feature === "object");
}
...
function classifyPoint(turnType: number, description: string): StepKind {
  ...
  if (turnType === 233 || turnType === 235 || turnType === 236 || turnType === 238) return "stairs";
